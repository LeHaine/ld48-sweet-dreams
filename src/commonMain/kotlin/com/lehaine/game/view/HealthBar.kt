...
class HealthBar(private val initWidth: Double, private val initHeight: Double) : Container() {
    ...
    val base = roundRect(initWidth + 2, initHeight + 2, 3.0, 3.0, Colors.BLACK)
    val red = roundRect(initWidth, initHeight, 3.0, 3.0, Colors["#7d0000"]) {
        centerOn(base)
    }
    val green = roundRect(initWidth, initHeight, 3.0, 3.0, Colors["#066d00"]) {
        centerOn(base)
        alignLeftToLeftOf(base, 1)
    }
